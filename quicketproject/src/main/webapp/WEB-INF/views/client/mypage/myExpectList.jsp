	...
	<body>
		<div class="contentContainer container">
			...
			<div class="communityTitle">
				<h1>기대평 모음</h1>
				<p>
					고객님께서 남겨주신 기대평을 확인할 수 있습니다.<br/>
					QUICKet 운영정책에 위반되거나, 기대평의 성격에 맞지 않는 글은 고객님께 사전통보 없이 삭제될 수 있습니다.
				</p>
			</div>
			
			<%-- ================= 리스트 시작 ================= --%>
			<div id="qnaList" class="table-height">
				<table summary="기대평 리스트" class="table">
					<thead>
						<tr>
							<th class="text-center col-md-4">공연명</th>
							<th class="text-center col-md-4">제목</th>
							<th data-value="i_regidate" class="order text-center col-md-4">작성일</th>
						</tr>
					</thead>
					<tbody id="list">
						<!-- 데이터 출력 -->
						...
						<c:choose>
							<c:when test="${not empty expectList}">
								<c:forEach var="expect" items="${expectList}" varStatus="status">
									<tr class="text-center" data-num="${expect.ex_no}">
										...
										<td class="text-center">${expect.s_name}</td>
										<td class="goDetail">
											<a href="/showDetail?s_num=${expect.s_num}#expectBox">${expect.ex_title}</a>
										</td>
							...
							<c:otherwise>
								<tr>
									<td colspan="3" class="tac text-center">등록된 기대평이 존재하지 않습니다.</td>
								</tr>
							</c:otherwise>
						</c:choose>
					</tbody>
